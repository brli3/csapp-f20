...
{	
    ...
    while (rover != NULL) {
        if (strcmp(rover->id, id) == 0) return rover;
        rover = rover->next;
    }
    return NULL;
}
...
int enqueue(boundary* head, char* id, void* data, int size)
{
    ...
    new_node->id = malloc((strlen(id) + 1) * sizeof(char));
...
void add_node(boundary* head, node* new_node)
{
    ...
    node* tail = head->other_bound->queue_pointer;
    tail->next = new_node;
    new_node->prev = tail;
    tail = new_node;
    new_node->next = NULL;
